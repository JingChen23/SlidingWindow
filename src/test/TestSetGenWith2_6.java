...
public class TestSetGenWith2_6 {	
//	原始数据转为训练集
	private int windowSize = 200;
	private double suspiciousAmt1 = 1999.19995;
	private double suspiciousAmt2 = 999.59998;
	
	public void testSetGenerator(BufferedReader br, BufferedWriter bw){
		try {			
...
//              变量从字符串中剥离            
		        ...
		        if (transMoney == null || transMoney.equals("") || transMoney.isEmpty()) {
		        	transMoney = "0";
		        }
...
//	    		除掉垃圾数据
//	            if(sign<7 && sign>1) fuck = "1";	            	  
//	                                    变量写入data object(InfoDao)
	            if(sign>6) info.setSign("1");
	            else info.setSign("0");
...
//		        	写入数据对象，为特征集的生成做准备
		        	...
		        	info.setAvg_amt_window(df.format(transAmt));
		        	info.setVar_amt_window(df.format(transAmt));
		        	info.setAvg_time_window("600");
		        	info.setVar_time_window("600");
	            }
//	            如果不是第一条的话
	            else{
//	            	起手先算当日累计金额
...
//		        	更新列表之后,计算最新的均值和方差
	            ...
	            +","+info.getAvg_time_window() +","+ info.getVar_time_window() +","+ info.getAvg_amt_window() +","+ info.getVar_amt_window() +","+ info.getSign();
	            bw.write(str1);
	            bw.newLine();
	        }
	...
	public static void main(String[] args) {
		TestSetGenWith2_6 dp = new TestSetGenWith2_6();
		try {
			FileReader reader = new FileReader("D://Jworkspace/6.csv");
	        BufferedReader br = new BufferedReader(reader);
	        FileWriter writer = new FileWriter("D://Jworkspace/6test.csv");
	        BufferedWriter bw = new BufferedWriter(writer);
			dp.testSetGenerator(br,bw);
			br.close();
			bw.close();
		} catch (Exception e) {
			e.printStackTrace();
